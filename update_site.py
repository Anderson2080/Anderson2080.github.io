...
def get_latest_news():
    feeds = [
        "https://36kr.com/feed", 
        "https://www.solidot.org/index.rss" # 增加了一个极客向的科技源，扩大搜索面
    ]
    news_items = []
    for url in feeds:
        try:
            parsed = feedparser.parse(url)
            # 抓取前8条，两个源共16条，给AI足够的信息量去筛选三大领域
...
def generate_radar_html(news_text):
    ...
    2. 专家级评价：用极其权威、犀利、穿透商业/技术本质的语气进行点评（100-150字）。不要平铺直叙，要反共识、讲出内行门道。
    3. 兜底机制：如果今天的新闻中确实没有某个领域的直接新闻，请你动用专家的先验知识，结合行业当前最大的痛点，自己撰写一条该领域的硬核前瞻研判（此时链接可以随便配一个相关的，或者留空用 # ）。
